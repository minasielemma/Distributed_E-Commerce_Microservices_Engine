...
import { chatService, orderService } from '../services/apiServices';
...
const MediaAttachmentViewer = ({ roomId, fileId, isMe, fileNameHint }) => {
  ...
  if (loading) {
    return (
      <div className="p-2.5 bg-black/20 border border-white/10 rounded-xl flex items-center gap-2 text-xs text-slate-300 animate-pulse">
        <FileText size={16} className="animate-spin text-indigo-400" />
  ...
  if (error) {
    return (
      <div className="p-2.5 bg-rose-950/40 border border-rose-500/30 rounded-xl flex items-center justify-between text-xs text-rose-300">
  ...
  if (isImage && blobUrl) {
    return (
      <div className="mt-1 space-y-1">
        <div className="relative group/img overflow-hidden rounded-xl border border-white/15 bg-black/40 inline-block">
          <img
            src={blobUrl}
            alt={fileName}
          ...
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center gap-2">
            ...
            <button
              type="button"
              onClick={handleDownload}
              className="p-2 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white backdrop-blur-md transition-all"
  ...
  return (
    <div className={`p-2.5 rounded-xl border flex items-center justify-between gap-3 ${
      ...
      <div className="flex items-center gap-2.5 min-w-0">
        <div className="p-2 rounded-lg bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 shrink-0">
          <FileText size={18} />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-xs font-bold truncate text-white" title={fileName}>
            {fileName}
          </p>
      ...
      <button
        ...
        className="px-2.5 py-1.5 rounded-lg bg-indigo-500/20 hover:bg-indigo-500/30 border border-indigo-500/40 text-indigo-200 hover:text-white text-xs font-semibold transition-all flex items-center gap-1.5 shrink-0"
...
export const ChatPage = () => {
  ...
  return (
    <div className="w-full h-[calc(100vh-2rem)] flex flex-col p-3 sm:p-4 md:p-6 overflow-hidden">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0 mb-3 sm:mb-4">
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-white tracking-tight flex items-center gap-2 sm:gap-3">
            <MessageSquare className="text-indigo-400 w-6 h-6 sm:w-7 sm:h-7" /> Real-Time Live Messaging
          </h1>
        ...
        <button
          ...
          className="btn-primary text-xs flex items-center gap-2 shadow-lg shadow-indigo-500/25 shrink-0"
        >
      ...
      <div className="glass-panel flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-4 overflow-hidden border border-white/10 shadow-2xl">
        {/* Left Sidebar: Rooms List */}
        <div className={`lg:col-span-1 border-r border-white/10 flex flex-col h-full overflow-hidden bg-slate-950/40 ${showMobileChatRoom ? 'hidden lg:flex' : 'flex'}`}>
          <div className="p-3 sm:p-4 border-b border-white/10 space-y-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900/80 border border-white/10 rounded-xl pl-9 pr-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-white/5">
            {loadingList ? (
              <div className="p-6 text-center text-xs text-slate-400">Loading rooms...</div>
            ) : filteredRooms.length === 0 ? (
              <div className="p-6 text-center text-xs text-slate-500">No chat rooms found.</div>
            ) : (
              filteredRooms.map((r) => {
                ...
                return (
                  ...
                  >
                    <div className="w-9 h-9 rounded-xl bg-indigo-500/20 text-indigo-300 flex items-center justify-center font-bold text-xs shrink-0 relative">
        ...
        <div className={`lg:col-span-3 flex flex-col h-full min-h-0 overflow-hidden bg-slate-900/30 ${!showMobileChatRoom ? 'hidden lg:flex' : 'flex'}`}>
          {selectedId && room ? (
            <>
              {/* Room Header */}
              <div className="p-3 sm:p-4 bg-slate-950/80 border-b border-white/10 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  <button
                    onClick={() => setShowMobileChatRoom(false)}
                    className="lg:hidden p-1.5 rounded-lg bg-slate-800 text-slate-300 hover:text-white shrink-0"
                    title="Back to Rooms"
                  >
                    <ChevronLeft size={18} />
                  </button>
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-indigo-600/30 text-indigo-400 flex items-center justify-center font-black shrink-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-xs sm:text-sm font-extrabold text-white flex items-center gap-2 truncate">
                      {room.name}
                    </h3>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 text-xs font-bold transition-all flex items-center gap-1.5"
                  >
              ...
              {/* Participants Bar */}
              <div className="px-4 py-2 bg-slate-950/60 border-b border-white/5 flex items-center gap-2 overflow-x-auto custom-scrollbar shrink-0 text-[11px]">
                <span className="text-slate-500 font-bold uppercase text-[9px]">Members:</span>
                {participants.map((p) => {
                  ...
                  return (
                    <span
                      key={p.id}
                      className="px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-slate-300 flex items-center gap-1.5"
                    >
                      <span
                      ...
                      <span className="text-[9px] text-indigo-400 font-mono">({p.role})</span>
                      {p.user_id !== user?.id && (
                        <button
              ...
              <div
                ...
                className="flex-1 min-h-0 p-6 overflow-y-auto custom-scrollbar space-y-4 bg-slate-950/40"
              >
                {loadingMore && (
                  <div className="flex items-center justify-center py-2 gap-2 text-xs text-indigo-400 font-semibold animate-pulse">
                    ...
                    if (isSystem) {
                      return (
                        <div key={msg.id} className="text-center my-2">
                          <span className="px-3 py-1 rounded-full bg-slate-800/60 border border-slate-700/50 text-slate-400 text-xs font-medium">
                            {msg.content}
                          </span>
                        </div>
                      );
                    ...
                    return (
                      ...
                      >
                        <div className={`flex items-center gap-2 mb-1 ${isMe ? 'flex-row-reverse text-right' : 'flex-row text-left'}`}>
                          <span className={`text-xs font-bold ${isMe ? 'text-indigo-400' : 'text-slate-400'}`}>
                        ...
                        <div className={`relative flex items-center gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'} max-w-[85%] sm:max-w-md`}>
                          ...
                          <div className="relative opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                            <button
                              ...
                              className="p-1 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white transition-all text-xs"
                              title="React with Emoji"
                            >
                              <Smile size={14} />
                            </button>
                            {activeEmojiPickerMsgId === msg.id && (
                              <div className={`absolute z-30 bottom-full mb-1.5 px-2 py-1 bg-slate-900 border border-white/15 rounded-2xl flex items-center gap-1.5 shadow-2xl whitespace-nowrap ${isMe ? 'right-0' : 'left-0'}`}>
                                {EMOJI_OPTIONS.map((e) => (
                                  <button
                                    key={e}
                                    type="button"
                          ...
                          <div
                            className={`p-3.5 rounded-2xl text-sm leading-relaxed space-y-2 relative ${
                              isMe
                                ? 'bg-indigo-600 text-white rounded-tr-none shadow-lg shadow-indigo-600/25 ml-auto'
                                : 'bg-slate-900 border border-white/10 text-slate-200 rounded-tl-none mr-auto'
                            }`}
                          >
                            ...
                            {isMe && (
                              <div className="flex justify-end items-center text-[10px] text-indigo-200/80 gap-1 mt-1">
                                {recStatus === 'READ' ? (
                                  <CheckCheck size={14} className="text-emerald-300" title="Read" />
                                ) : recStatus === 'DELIVERED' ? (
                                  <CheckCheck size={14} className="text-indigo-200" title="Delivered" />
                                ) : (
                                  <Check size={14} className="text-indigo-300/70" title="Sent" />
                                )}
                              </div>
                            )}
                          </div>
                          ...
                          <div className={`flex flex-wrap gap-1 mt-1.5 ${isMe ? 'justify-end' : 'justify-start'}`}>
                            {msg.reactions.map((r) => (
                              <button
                                ...
                                className="px-2 py-0.5 rounded-full bg-slate-800/90 border border-white/10 text-[11px] text-slate-300 flex items-center gap-1 hover:bg-slate-700 transition-all"
                              >
                                <span>{r.emoji}</span>
              ...
              {/* Input Bar */}
              <div className="p-4 bg-slate-900/80 border-t border-white/10 flex flex-col gap-2 shrink-0 mt-auto">
                {selectedFile && (
                  <div className="px-3 py-1.5 bg-indigo-950/60 border border-indigo-500/30 rounded-xl flex items-center justify-between text-xs text-indigo-300">
                ...
                <form onSubmit={handleSend} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-slate-950/90 border border-white/15 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                  />

                  <button
                    type="submit"
                    disabled={sending || (!inputMsg.trim() && !selectedFile)}
                    className="px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold text-xs transition-all shadow-lg shadow-indigo-500/25 flex items-center gap-2"
                  >
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-500 gap-3 p-8">
              ...
              <button
                onClick={openCreateModal}
                className="mt-2 px-4 py-2 rounded-xl bg-indigo-600 text-white text-xs font-bold shadow-lg shadow-indigo-500/25"
              >
                + Create Chat Room
              </button>
            </div>
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-black text-white flex items-center gap-2">
                <Sparkles className="text-indigo-400" size={20} /> Quick-Start Chat Room
              </h2>
            ...
            {/* Quick Templates */}
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-2">Preset Templates:</label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => applyTemplate('ORDER_SUPPORT')}
                  className="p-2.5 rounded-xl bg-white/5 hover:bg-indigo-600/20 border border-white/10 hover:border-indigo-500/40 text-left transition-all"
                >
                  <p className="text-xs font-bold text-white">🛒 Order Support</p>
                  <p className="text-[10px] text-slate-400">Link to customer order</p>
                </button>

                <button
                  type="button"
                  onClick={() => applyTemplate('PAYMENT')}
                  className="p-2.5 rounded-xl bg-white/5 hover:bg-purple-600/20 border border-white/10 hover:border-purple-500/40 text-left transition-all"
                >
                  <p className="text-xs font-bold text-white">💳 Payment Inquiry</p>
                  <p className="text-[10px] text-slate-400">Billing & Invoices</p>
                </button>

                <button
                  type="button"
                  onClick={() => applyTemplate('PRODUCT')}
                  className="p-2.5 rounded-xl bg-white/5 hover:bg-pink-600/20 border border-white/10 hover:border-pink-500/40 text-left transition-all"
                >
                  <p className="text-xs font-bold text-white">📦 Product Support</p>
                  <p className="text-[10px] text-slate-400">Returns & Questions</p>
                </button>

                <button
                  type="button"
                  onClick={() => applyTemplate('TEAM')}
                  className="p-2.5 rounded-xl bg-white/5 hover:bg-emerald-600/20 border border-white/10 hover:border-emerald-500/40 text-left transition-all"
                >
                  <p className="text-xs font-bold text-white">👥 Store Operations</p>
                  <p className="text-[10px] text-slate-400">Team group channel</p>
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleCreateRoom} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 bg-slate-950 border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:border-indigo-500"
                >
                  <option value="ORDER_SUPPORT">🛒 Order Support Room (Linked to Store Order)</option>
                  <option value="GROUP">👥 Store Operations & Team Group</option>
                  <option value="DIRECT">💬 Direct 1-on-1 Customer Support</option>
                </select>
              </div>

              {newRoomType === 'ORDER_SUPPORT' && (
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">Link to Store Order (Optional)</label>
                  {loadingOrders ? (
                    <div className="text-xs text-slate-500 py-1">Loading recent orders...</div>
                  ) : (
                    <select
                      value={selectedOrder}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Room Display Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Initial Greeting Message (Optional)</label>
                <input
                  type="text"
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-white/10">
                <button
                  ...
                  className="px-4 py-2 bg-white/5 hover:bg-white/10 text-slate-300 font-bold text-xs rounded-xl"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-extrabold text-xs rounded-xl shadow-lg shadow-indigo-500/25"
                >
      ...
      {showAddUserModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-black text-white flex items-center gap-2">
                <UserPlus className="text-indigo-400" size={20} /> Add Room Participant
              </h2>
            ...
            <form onSubmit={handleAddParticipant} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Username (or User ID)</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2.5 bg-slate-950 border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:border-indigo-500 font-mono"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Participant Display Name (Optional)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Participant Role</label>
                <select
                  value={addUserRole}
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-white/10">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-500/25"
                >
                  Add Participant
                </button>
              </div>
