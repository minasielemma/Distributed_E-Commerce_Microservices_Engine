...
logger = logging.getLogger("payment_outbox_publisher")
...
def publish_pending_events():
    ...
    try:
        ...
        logger.error(f"[Payment Outbox Publisher] Failed to connect to Kafka at {bootstrap_servers}: {e}")
        return
    ...
    for ev in pending_events:
        ...
        try:
            ...
            logger.info(
                f"[Payment Outbox Publisher] Published event {ev.id} ({ev.event_type}) to "
            ...
            logger.error(f"[Payment Outbox Publisher] Kafka error publishing event {ev.id}: {k_err}")
        except Exception as err:
            logger.error(f"[Payment Outbox Publisher] Unexpected error publishing event {ev.id}: {err}")
...
def run_loop(poll_interval=3):
    logger.info("[Payment Outbox Publisher] Starting loop...")
    while True:
        try:
            publish_pending_events()
        except Exception as loop_err:
            logger.error(f"[Payment Outbox Publisher] Error in loop: {loop_err}")
