...
class CreateCheckoutSessionView (APIView ):
    ...
    def post (self ,request ):
        ...
        if existing_payment and existing_payment .polar_checkout_url and "polar_chk_"not in existing_payment .polar_checkout_id and "https://polar.sh/checkout/polar_chk_"not in existing_payment .polar_checkout_url :
            ...
            'checkout_url':existing_payment .polar_checkout_url 
        ...
        if 'error'in session_info :
            ...
            with transaction .atomic ():
                ...
                'provider':'SIMULATED',
                'status':'PAID',
                'polar_checkout_id':f"sim_chk_{uuid .uuid4 ().hex [:10 ]}"
                }
                )
                ...
                if discount_code :
                    try :
                        from .utils import make_service_token 
                        s_token =make_service_token ()
                        requests .post (
                        f'http://catalog_service:8000/api/catalog/coupons/confirm/{order_id }/',
                        headers ={'X-Service-Token':s_token ,'Content-Type':'application/json'},
                        timeout =5 
                        )
                ...
                try :
                    ...
                    'X-Service-Token':s_token ,
                    'Content-Type':'application/json'
                    }
                    requests .post (
                    f"http://order_service:8000/api/orders/{order_id }/mark-paid/",
                    json ={'status':'PAID'},
                    headers =headers ,
                    timeout =5 
                    )
        ...
        with transaction.atomic():
            payment, created = Payment.objects.select_for_update().get_or_create(
                ...
                defaults={
                    ...
                    'provider': 'POLAR',
                    'status': 'PENDING',
                    'polar_checkout_id': session_info['checkout_id'],
                    'polar_checkout_url': session_info['checkout_url']
                }
            )
            if not created:
        ...
        return Response ({
        'message':'Polar Checkout session created successfully',
        'payment':PaymentSerializer (payment ).data ,
    ...
    def post (self ,request ):
        ...
        with transaction .atomic ():
            ...
            if not payment :
                ...
                amount =Decimal (str (request .data .get ('amount')or '10.00')),
                provider ='SIMULATED',
                status ='PAID',
                polar_checkout_id =f"sim_chk_{uuid .uuid4 ().hex [:10 ]}"
                )
            ...
            if payment .status =='PAID':
                return Response ({
                'message':f'Payment for order {order_id } already confirmed (idempotent)',
                'payment':PaymentSerializer (payment ).data ,
                'status':'PAID'
                },status =status .HTTP_200_OK )
...
class PolarWebhookView (APIView ):
    ...
    def post (self ,request ):
        provider =PolarPaymentProvider ()
        sig = (
            request .META .get ('HTTP_WEBHOOK_SIGNATURE')
            or request .META .get ('HTTP_X_POLAR_SIGNATURE')
            or request .META .get ('HTTP_POLAR_SIGNATURE')
            or request .headers .get ('webhook-signature')
            or request .headers .get ('x-polar-signature')
        )
        msg_id = (
            request .META .get ('HTTP_WEBHOOK_ID')
        ...
        # Ignore webhooks for non-terminal / unpaid states like 'open', 'pending', 'failed', 'cancelled'
        ...
        order_id =(
        payload_data .get ('metadata',{}).get ('order_id')
        or payload_data .get ('order_id')
        or data .get ('order_id')
        or payload_data .get ('custom_field_data',{}).get ('order_id')
        )
        ...
        if payment :
            with transaction .atomic ():
                ...
                if payment .status =='PAID':
                    return Response (
                    {'status':'already_paid','message':'Payment already processed (idempotent)'},
                    status =status .HTTP_200_OK 
                    )
                ...
                try :
                    ...
                    requests .post (
                    f"http://order_service:8000/api/orders/{payment.order_id }/mark-paid/",
                    json ={'status':'PAID'},
                    headers =headers ,
                    timeout =5 
                    )
            ...
            if payment .discount_code :
                try :
                    ...
                    s_token =make_service_token ()
                    requests .post (
                    f'http://catalog_service:8000/api/catalog/coupons/confirm/{payment .order_id }/',
                    headers ={'X-Service-Token':s_token ,'Content-Type':'application/json'},
                    timeout =5 
                    )
                except Exception as coupon_err :
