...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
    """Consumer for real-time notification push to authenticated clients using 2-step auth."""
    ...
    async def notification_message(self, event):
        """Handler for 'notification.message' group events from Channel Layer."""
        await self.send_json({
            'type': 'notification',
            'data': event.get('data', {}),
        })
...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    """Consumer for real-time chat room communication using 2-step auth."""
    ...
    async def _complete_connection(self, user):
        ...
        await self.send_json({
            'type': 'connection_established',
            'status': 'authenticated',
            'message': f"Connected to chat conversation {self.conversation_id}",
        })

    async def disconnect(self, close_code):
        if hasattr(self, 'room_group_name') and getattr(self, 'is_authenticated', False):
    ...
    async def chat_message(self, event):
        """Handler for 'chat_message' group events."""
        await self.send_json({
            'type': 'chat_message',
            'data': event.get('data', {}),
        })
